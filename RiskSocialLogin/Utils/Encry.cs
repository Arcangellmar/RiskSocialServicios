...
namespace RiskSocialLogin.Utils
{
    public class Encry
    {
        ...
        public static string Decry(string password)
        {
            //using (var sha256 = SHA256.Create())
            //{
            //    byte[] passwordBytes = System.Text.Encoding.UTF8.GetBytes(password);
            ...
            using (var kmsClient = new AmazonKeyManagementServiceClient())
            {
                var descifradoRequest = new DecryptRequest
                {
                    CiphertextBlob = new MemoryStream(Convert.FromBase64String(password))
                };

                var descifradoResponse = kmsClient.DecryptAsync(descifradoRequest).Result;
                var textoDescifrado = Encoding.UTF8.GetString(descifradoResponse.Plaintext.ToArray());
